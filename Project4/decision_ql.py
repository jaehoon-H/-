...
class QLearningDecisionPolicy:
    def __init__(self, epsilon, gamma, lr, actions, input_dim, model_dir):
        ...
        fc1 = tf.layers.dense(self.x, 32, activation=tf.nn.sigmoid)
        fc2 = tf.layers.dense(fc1, 16, activation=tf.nn.sigmoid)
        fc3 = tf.layers.dense(fc2, 8, activation=tf.nn.sigmoid)
        fc4 = tf.layers.dense(fc3, 4, activation=tf.nn.sigmoid)
        self.q = tf.layers.dense(fc4, output_dim)
